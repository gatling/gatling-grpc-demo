...
export default simulation((setUp) => {
  ...
  const clientStream = grpc("Compute Average")
    ...
    .check(
      statusCode().is("OK"),
      response((response) => response.average).saveAs("average")
    );

  const clientStreaming = scenario("Calculator Client Streaming").exec(
    clientStream.start(),
    repeat(10).on(
      clientStream.send((_: Session) => {
        const number = Math.floor(Math.random() * 1000);
        return { number };
      })
    ),
    clientStream.halfClose(),
    clientStream.awaitStreamEnd((main: Session, forked: Session) => {
      const average = forked.get<number>("average");
      return main.set("average", average);
    }),
    exec((session: Session) => {
      const average = session.get<double>("average");
  ...
  const bidirectionalStreaming = scenario("Calculator Bidirectional Streaming").exec(
    ...
    repeat(10).on(
      bidirectionalStream.send((_: Session) => {
        const number = Math.floor(Math.random() * 1000);
        return { number };
      })
    ),
    bidirectionalStream.halfClose(),
    bidirectionalStream.awaitStreamEnd((main: Session, forked: Session) => {
      const latestMaximum = forked.get<number>("maximum");
      return main.set("maximum", latestMaximum);
    }),
    exec((session: Session) => {
      const maximum = session.get<number>("maximum");
